...
func _transition_to_next_state(target_state_path: String, data: Dictionary = {}) -> void:
	...
	if not state.can_enter(previous_state_path, data):
		printerr(owner.name + ": Trying to transition to state " + target_state_path + " but it is not allowed.")
		return
	state.enter(previous_state_path, data)
	print(owner.name + ": made transition to state " + target_state_path)
	state_changed.emit()
